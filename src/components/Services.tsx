    ...
    <section id="pakalpojumi" className="w-full py-16 md:py-20 bg-[#f9f9f9] scroll-mt-20">
      <div className="container mx-auto px-4 md:px-8 lg:px-[75px]">
        ...
        <div className="hidden lg:grid lg:grid-cols-3 gap-[24px] items-start">
          ...
          {/* Правые карточки */}
          <div className="grid grid-cols-2 gap-x-[24px] gap-y-[18px] col-span-2 self-start">
...
const ServiceCard = ({ image, title, description, href, large }: any) => {
  return (
    <div className={`w-full bg-white rounded-none overflow-hidden flex flex-col`}>
      {/* Изображение */}
      <div className={`${large ? "h-[480px]" : "h-[250px]"} w-full`}>
      ...
      <div className="px-6 py-4 flex flex-col justify-between flex-grow">
        <div>
          ...
          <p className="font-sans text-sm text-muted-foreground leading-snug">{description}</p>
        </div>

        <a
          href={href}
          className="mt-4 group relative inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-primary/10 text-primary font-sans font-semibold transition ease-out duration-200 hover:scale-[1.02] active:scale-[0.97] before:absolute before:inset-0 before:-translate-x-full before:bg-gradient-to-r before:from-transparent before:via-white/20 before:to-transparent group-hover:before:translate-x-full before:transition-transform before:duration-700"
        >
